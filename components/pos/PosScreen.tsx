                  ...
                  {currentOrder.map((item) => (
                    <li key={item.instanceId} className="py-3 flex justify-between items-start">
                      ...
                      <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => updateQuantity(item.instanceId, -item.quantity)} aria-label={`Remove ${item.name} from order`} className="text-red-500 hover:text-red-400 p-1 focus:ring-2 focus:ring-red-400 focus:outline-none rounded"><TrashIcon className="w-4 h-4" /></button>
                      </div>
                    </li>
                  ))}
                </ul>
