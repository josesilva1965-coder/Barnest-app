...
const MenuItemCard: React.FC<{
    ...
    return (
        <button
            type="button"
            className="w-full h-full text-left group focus:outline-none rounded-lg focus:ring-2 focus:ring-brand-secondary focus:ring-offset-2 focus:ring-offset-brand-dark"
            onClick={() => onSelect(item)}
            aria-label={`Add ${item.name} to order for $${item.price.toFixed(2)}`}
        >
            <Card className="flex flex-col text-center items-center p-2 h-full transition-all duration-200 border-brand-primary group-hover:border-brand-secondary">
                  ...
                  {currentOrder.map((item) => (
                    <li key={item.instanceId} className="py-3 flex justify-between items-start">
                      ...
                      <div className="flex items-center gap-2">
                          ...
                          <span className="font-bold w-6 text-center" aria-live="polite" aria-label={`${item.quantity} items`}>{item.quantity}</span>
                          <button
                              onClick={() => updateQuantity(item.instanceId, 1)}
                              className="p-1 rounded-full bg-brand-primary hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-brand-secondary"
