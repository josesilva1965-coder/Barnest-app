                  ...
                  {currentOrder.map((item) => (
                    <li key={item.instanceId} className="py-3 flex justify-between items-start">
                      ...
                      <div className="flex items-center gap-2">
                          ...
                          <button aria-label={`Remove ${item.name}`} onClick={() => updateQuantity(item.instanceId, -item.quantity)} className="text-red-500 hover:text-red-400 p-1 focus-visible:ring-2 focus-visible:ring-brand-secondary focus-visible:outline-none"><TrashIcon className="w-4 h-4" /></button>
                      </div>
                    </li>
                  ))}
                </ul>
