                ...
                <ul className="divide-y divide-brand-primary">
                  {currentOrder.map((item) => (
                    <li key={item.instanceId} className="py-3 flex justify-between items-start">
                      ...
                      <div className="flex items-center gap-2">
                          <button aria-label={`Decrease quantity of ${item.name}`} onClick={() => updateQuantity(item.instanceId, -1)} className="p-1 rounded-full bg-brand-primary hover:bg-gray-600 focus-visible:ring-2 focus-visible:ring-brand-secondary focus-visible:outline-none"><MinusIcon className="w-4 h-4" /></button>
                          <span className="font-bold w-6 text-center">{item.quantity}</span>
                          <button aria-label={`Increase quantity of ${item.name}`} onClick={() => updateQuantity(item.instanceId, 1)} className="p-1 rounded-full bg-brand-primary hover:bg-gray-600 focus-visible:ring-2 focus-visible:ring-brand-secondary focus-visible:outline-none"><PlusIcon className="w-4 h-4" /></button>
                          <button aria-label={`Remove ${item.name} from order`} onClick={() => updateQuantity(item.instanceId, -item.quantity)} className="text-red-500 hover:text-red-400 p-1 rounded-full focus-visible:ring-2 focus-visible:ring-brand-secondary focus-visible:outline-none"><TrashIcon className="w-4 h-4" /></button>
                      </div>
                    </li>
                  ))}
                </ul>
