                            ...
                            {newItemsInCart.map(item => (
                                <div key={item.instanceId} className="flex justify-between items-center bg-brand-primary/50 p-2 rounded">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button aria-label="Decrease quantity" onClick={() => updateNewItemQuantity(item.instanceId, -1)} className="p-1 rounded-full bg-brand-primary hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-brand-secondary"><MinusIcon className="w-4 h-4" /></button>
                                        <span className="font-bold w-6 text-center" aria-live="polite">{item.quantity}</span>
                                        <button aria-label="Increase quantity" onClick={() => updateNewItemQuantity(item.instanceId, 1)} className="p-1 rounded-full bg-brand-primary hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-brand-secondary"><PlusIcon className="w-4 h-4" /></button>
                                    </div>
                                </div>
                            ))}
                        </div>
