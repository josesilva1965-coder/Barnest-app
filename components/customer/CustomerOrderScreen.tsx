        ...
        <div className="min-h-screen bg-brand-dark text-brand-light p-4 pb-24">
            ...
            {newItemsInCart.length > 0 && (
                <button
                    ...
                    aria-label={`${newItemsInCart.reduce((sum, item) => sum + item.quantity, 0)} items in cart, click to view`}
                    className="fixed bottom-6 right-6 bg-brand-secondary text-white rounded-full p-4 shadow-lg flex items-center gap-2 animate-bounce focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-light focus-visible:ring-offset-2 focus-visible:ring-offset-brand-dark"
                >
                    <ShoppingCartIcon className="w-6 h-6" />
                    <span aria-live="polite" aria-atomic="true" className="font-bold">{newItemsInCart.reduce((sum, item) => sum + item.quantity, 0)}</span>
                </button>
            )}
                            ...
                            {newItemsInCart.map(item => (
                                <div key={item.instanceId} className="flex justify-between items-center bg-brand-primary/50 p-2 rounded">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button aria-label={`Decrease quantity of ${item.name}`} onClick={() => updateNewItemQuantity(item.instanceId, -1)} className="p-1 rounded-full bg-brand-primary hover:bg-gray-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-secondary"><MinusIcon className="w-4 h-4" /></button>
                                        <span aria-live="polite" aria-atomic="true" className="font-bold w-6 text-center">{item.quantity}</span>
                                        <button aria-label={`Increase quantity of ${item.name}`} onClick={() => updateNewItemQuantity(item.instanceId, 1)} className="p-1 rounded-full bg-brand-primary hover:bg-gray-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-secondary"><PlusIcon className="w-4 h-4" /></button>
                                    </div>
                                </div>
                            ))}
                        </div>
