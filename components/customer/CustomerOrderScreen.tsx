        ...
        <div className="min-h-screen bg-brand-dark text-brand-light p-4 pb-24">
            ...
            {newItemsInCart.length > 0 && (
                <button
                    ...
                    className="fixed bottom-6 right-6 bg-brand-secondary text-white rounded-full p-4 shadow-lg flex items-center gap-2 animate-bounce focus:ring-2 focus:ring-brand-primary focus:outline-none"
                            ...
                            {newItemsInCart.map(item => (
                                <div key={item.instanceId} className="flex justify-between items-center bg-brand-primary/50 p-2 rounded">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button onClick={() => updateNewItemQuantity(item.instanceId, -1)} className="p-1 rounded-full bg-brand-primary hover:bg-gray-600 focus:ring-2 focus:ring-brand-secondary focus:outline-none" aria-label={`Decrease quantity of ${item.name}`}><MinusIcon className="w-4 h-4" /></button>
                                        <span className="font-bold w-6 text-center" aria-live="polite" aria-atomic="true">{item.quantity}</span>
                                        <button onClick={() => updateNewItemQuantity(item.instanceId, 1)} className="p-1 rounded-full bg-brand-primary hover:bg-gray-600 focus:ring-2 focus:ring-brand-secondary focus:outline-none" aria-label={`Increase quantity of ${item.name}`}><PlusIcon className="w-4 h-4" /></button>
                                    </div>
                                </div>
                            ))}
                        </div>
