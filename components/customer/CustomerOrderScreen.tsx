                            ...
                            {newItemsInCart.map(item => (
                                <div key={item.instanceId} className="flex justify-between items-center bg-brand-primary/50 p-2 rounded">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button onClick={() => updateNewItemQuantity(item.instanceId, -1)} aria-label={`Decrease quantity of ${item.name}`} title={`Decrease quantity of ${item.name}`} className="p-1 rounded-full bg-brand-primary hover:bg-gray-600 focus-visible:ring-2 focus-visible:ring-brand-secondary focus-visible:outline-none"><MinusIcon className="w-4 h-4" /></button>
                                        <span className="font-bold w-6 text-center">{item.quantity}</span>
                                        <button onClick={() => updateNewItemQuantity(item.instanceId, 1)} aria-label={`Increase quantity of ${item.name}`} title={`Increase quantity of ${item.name}`} className="p-1 rounded-full bg-brand-primary hover:bg-gray-600 focus-visible:ring-2 focus-visible:ring-brand-secondary focus-visible:outline-none"><PlusIcon className="w-4 h-4" /></button>
                                    </div>
                                </div>
                            ))}
                        </div>
