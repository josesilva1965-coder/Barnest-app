    ...
    return (
        <div className="min-h-screen bg-brand-dark text-brand-light p-4 pb-24">
            ...
            {newItemsInCart.length > 0 && (
                <button
                    aria-label="View new items in cart"
                    onClick={() => setIsCartOpen(true)}
                    className="fixed bottom-6 right-6 bg-brand-secondary text-white rounded-full p-4 shadow-lg flex items-center gap-2 animate-bounce focus:outline-none focus:ring-4 focus:ring-brand-secondary focus:ring-opacity-50"
                >
                    ...
                    <span
                        ...
                        aria-live="polite"
                        aria-atomic="true"
                    >{newItemsInCart.reduce((sum, item) => sum + item.quantity, 0)}</span>
                </button>
            )}
