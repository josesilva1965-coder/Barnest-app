        ...
        <div className="min-h-screen bg-brand-dark text-brand-light p-4 pb-24">
            ...
            {newItemsInCart.length > 0 && (
                <button
                    ...
                    className="fixed bottom-6 right-6 bg-brand-secondary text-white rounded-full p-4 shadow-lg flex items-center gap-2 animate-bounce focus:outline-none focus:ring-4 focus:ring-white focus:ring-offset-2 focus:ring-offset-brand-dark"
                >
                            ...
                            {newItemsInCart.map(item => (
                                <div key={item.instanceId} className="flex justify-between items-center bg-brand-primary/50 p-2 rounded">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button aria-label={`Decrease quantity of ${item.name}`} onClick={() => updateNewItemQuantity(item.instanceId, -1)} className="p-1 rounded-full bg-brand-primary hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-brand-secondary"><MinusIcon className="w-4 h-4" /></button>
                                        <span className="font-bold w-6 text-center" aria-live="polite" aria-atomic="true">{item.quantity}</span>
                                        <button aria-label={`Increase quantity of ${item.name}`} onClick={() => updateNewItemQuantity(item.instanceId, 1)} className="p-1 rounded-full bg-brand-primary hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-brand-secondary"><PlusIcon className="w-4 h-4" /></button>
                                    </div>
                                </div>
                            ))}
                        </div>
